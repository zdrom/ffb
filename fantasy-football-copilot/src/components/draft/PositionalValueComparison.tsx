...
const PositionalValueComparison: React.FC = () => {
  ...
  // Memoize user team lookup
  const userTeam = useMemo(() => 
    state.teams.find(t => t.isUser), 
    [state.teams]
  );

  // Memoize VORP engine creation
  const vorpEngine = useMemo(() => {
    if (!userTeam || state.players.length === 0) return null;
    return new DynamicVORPEngine(state.players, state.settings, state.teams);
  }, [state.players.length, state.settings, state.teams, userTeam]);

  // Memoize top recommendation
  const topRecommendation = useMemo(() => {
    if (!userTeam || !vorpEngine) return null;
  ...
  }, [state.players, userTeam, state.teams, state.settings, state.currentPick, state.picks, vorpEngine]);

  // Memoize positional data calculations
  const { vorpByPosition, starterSatisfaction } = useMemo(() => {
    if (!vorpEngine || !userTeam) {
      return { vorpByPosition: new Map(), starterSatisfaction: new Map() };
    }
    ...
    return { vorpByPosition, starterSatisfaction };
  }, [state.players, state.teams, state.settings, vorpEngine, userTeam]);

  const getPositionColor = useCallback((position: Position) => {
      ...
      <div className="p-6">
        ...
        {topRecommendation && (
          <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center mb-2">
              ...
              <h3 className="text-lg font-semibold text-blue-900">Top VORP Recommendation</h3>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {positions.map(position => {
            ...
            const vorpValues = playersWithVORP.map((p: any) => p.vorp);
                  ...
                  {satisfaction && (
                    <div className="text-xs text-gray-500">
                      <div className="flex justify-end space-x-1 mb-1">
                        {satisfaction.distribution.map((count: any, starterCount: any) => (
                    ...
                    {nextFive.length > 0 && (
                      <div className="mb-4">
                        ...
                        <div className="space-y-1">
                          {playersWithVORP.map((playerData: any, index: any) => {
...
export default React.memo(PositionalValueComparison);